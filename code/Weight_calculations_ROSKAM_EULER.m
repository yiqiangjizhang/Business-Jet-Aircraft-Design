...
h_cruise_vector=[12000 12500 13000]; % [m]
...
MTOW_estimated = 27000 ; % MTOW estimation [kg]
...
MTOW_Euler_torenbeek=MTOW_estimated;
% Counter initialization
counter=0;
% Data storage matrix initialization
data_storage_Euler_torenbeek=[];
...
[C_D0,k] = Polar_parameters_function(MTOW_Euler_torenbeek,S,Wing_span);

% Mass at the beginning of the cruise stage
Mass_begin_cruise=fraction1*fraction2*fraction3*fraction4*MTOW_Euler_torenbeek;
...
% Euler resolution of this phase of the cruise
[~,~,fraction5(stage),Efficiency(stage,:)] = Euler_range_function(cj_cruise,M_cruise,C_D0,k,S,h_cruise,Mass_begin_cruise*9.81,R_stage,M);
...
% Euler resolution of the loiter
[~,~,fraction6] = Euler_autonomy_function(cj_loiter,M_loiter,C_D0,k,S,h_loiter,Mass_begin_loiter*9.81,Autonomy);

% Mass at the beginning of the reserve stage
Mass_begin_reserve=Mass_begin_loiter*fraction6;

% Euler resolution of the reserve stage
[~,~,fraction_reserve] = Euler_autonomy_function(cj_loiter,M_loiter,C_D0,k,S,h_reserve,Mass_begin_reserve*9.81,Autonomy);

% Mission fuel fraction
M_ff_Euler=fraction1*fraction2*fraction3*fraction4*fraction5_prod*fraction6*fraction_reserve*fraction7*fraction8

% Total fuel weight computation
W_f_Euler = (1-M_ff_Euler)*MTOW_Euler_torenbeek; 

% Operating empty weight computation (tentative)
OEW_tentative=MTOW_Euler_torenbeek-W_f_Euler-MPL;

% Allowable operating empty weight computation (according to Torenbeek criterion)
OEW_torenbeek = 0.2*MTOW_Euler_torenbeek + 2*engine_weight+ Delta_e+500;

% Relative deviation computation
OEW_torenbeek_deviation=abs(OEW_torenbeek-OEW_tentative)/OEW_torenbeek;

% Data storage
data_storage_Euler_torenbeek(1,counter)=OEW_tentative; % Tentative result
data_storage_Euler_torenbeek(2,counter)=OEW_torenbeek; % Torenbeek result
...
if OEW_torenbeek_deviation>0.0005
       ...
       if OEW_torenbeek>OEW_tentative 
            MTOW_Euler_torenbeek=MTOW_Euler_torenbeek+10;
       else
            MTOW_Euler_torenbeek=MTOW_Euler_torenbeek-10;
       end
...
plot([1:1:size(data_storage_Euler_torenbeek,2)], data_storage_Euler_torenbeek(1,:),'b')

plot([1:1:size(data_storage_Euler_torenbeek,2)], data_storage_Euler_torenbeek(2,:),'r')
...
error=true;
% Assignation of the estimated MTOW
MTOW_Euler_similarities=MTOW_estimated;
% Counter initialization
counter=0;
% Data storage matrix initialization
data_storage_Euler_similarities=[];
...
% Drag polar coefficients computation.
% It is only performed once, as it only depends on the MTOW
[C_D0,k] = Polar_parameters_function(MTOW_Euler_similarities,S,Wing_span);

% Mass at the beginning of the cruise stage
Mass_begin_cruise=fraction1*fraction2*fraction3*fraction4*MTOW_Euler_similarities;
...
% Total fuel weight computation
W_f_Euler = (1-M_ff_Euler)*MTOW_Euler_similarities; 

% Operating empty weight computation (tentative)
OEW_tentative=MTOW_Euler_similarities-W_f_Euler-MPL;

% Allowable operating empty weight computation (according to similarities criterion)
OEW_similarities=alpha*MTOW_Euler_similarities;

% Relative deviation computation
OEW_similarities_deviation=abs(OEW_similarities-OEW_tentative)/OEW_similarities;

% Data storage
data_storage_Euler_similarities(1,counter)=OEW_tentative; % Tentative result
data_storage_Euler_similarities(2,counter)=OEW_similarities; % Similarities result
...
if OEW_similarities_deviation>0.0005
       ...
       if OEW_similarities>OEW_tentative 
            MTOW_Euler_similarities=MTOW_Euler_similarities+10;
       else
            MTOW_Euler_similarities=MTOW_Euler_similarities-10;
       end
...
plot([1:1:size(data_storage_Euler_similarities,2)], data_storage_Euler_similarities(1,:),'b')
hold on
plot([1:1:size(data_storage_Euler_similarities,2)], data_storage_Euler_similarities(2,:),'r')
